  ...
  <div>
    <div>
      ...
      <div style="display: flex;margin-top: 20px">
        ...
        <div style="margin-top: 10px;margin-left: 250px;">查询内容</div>
        <el-input v-model="content" clearable style="width: 300px"></el-input>
        ...
        <el-button type="primary" style="height: 40px" @click="search">确定</el-button>
      </div>
    </div>
    <div class="d3-container">
      ...
      <div class="btns" v-if="buttonVisible">
        <el-button circle id="zoomIn" type="success" style="height: 40px;width: 40px">+</el-button>
        <el-button circle id="zoomOut" type="danger" style="height: 40px;width: 40px">-</el-button>
        <el-button id="reset">reset</el-button>
      </div>
      <svg />
    </div>
  </div>
...
import mock from './mockTest1.json'
...
export default {
  ...
  mounted () {
    this.setTypeColor()
    this.chooseValue = sessionStorage.getItem('author')
    this.content = sessionStorage.getItem('authorContent')
    this.initD3()
  },
  methods: {
    returnBack () {
      sessionStorage.removeItem('author')
      sessionStorage.removeItem('authorContent')
      this.$router.push('/')
    },
    search () {
      sessionStorage.setItem('author', this.chooseValue)
      sessionStorage.setItem('authorContent', this.content)
      location.reload()
    },
    handleTypeMouseout () {
    ...
    initD3 () {
      ...
      _this.$nextTick(() => {
        if (this.chooseValue === '1') {
          const nodeList = mock.data.nodes
          const relationShip = mock.data.relations
          for (let i = 0; i < nodeList.length; i++) {
            _this.nodes.push(nodeList[i])
          }
          for (let i = 0; i < relationShip.length; i++) {
            _this.links.push(relationShip[i])
          }
          updateObj.update({
            nodes: _this.nodes,
            links: _this.links
