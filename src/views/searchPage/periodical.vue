<template>
  <div>
    <div>
      <div style="font-weight: bolder;font-size: 32px">期刊查询</div>
      <div style="display: flex;margin-top: 20px">
        <el-button circle type="success" style="width: 50px;height: 50px;margin-left: 100px" @click="returnBack">返回</el-button>
        <div style="margin-top: 10px;margin-left: 150px;">年份</div>
        <el-input v-model="year" clearable style="width: 150px;margin-left: 10px"></el-input>
        <div style="margin-top: 10px;margin-left: 100px">期刊</div>
        <el-input v-model="venue" clearable style="width: 150px;margin-left: 10px"></el-input>
      ...
      <div v-if="timesVisible">
        <div style="margin-top: 50px;font-weight: bolder" >{{this.venue}}被引用的次数为: {{this.times}}</div>
        <p style="width: 700px;height: 700px"></p>
      </div>
    </div>
    <div>
    </div>
    <div class="d3-container">
      ...
      <div class="info" v-if="visibleList[0]">
        ...
        <div style="font-weight: bolder">年份: {{info.year}}</div>
      </div>
...
import {getPapersPublishedByVenueInYear, getAuthorsByVenue, getPaperInVenueIsCitedTimes} from '../../api/periodicalAPI'
...
export default {
  ...
  mounted () {
    ...
    this.venue = sessionStorage.getItem('venue')
  ...
  methods: {
    ...
    search () {
      ...
      if ((this.year === '' || this.year === null) && (this.chooseValue === '1')) {
    ...
    initD3 () {
      ...
      _this.$nextTick(() => {
        ...
        if (this.chooseValue === '1') {
          const param = {venue: this.venue, year: parseInt(this.year)}
          getPapersPublishedByVenueInYear(param).then(res => {
            ...
            _this.links = []
            const venueId = 1
            _this.nodes.push({id: 1, name: this.venue, label: 'venue'})
            ...
            for (let i = 0; i < _this.links.length; i++) {
              ...
              _this.links[i]['source'] = venueId
              _this.links[i]['target'] = _this.links[i].id
              _this.links[i]['relation'] = 'publish'
            }
            updateObj.update({
              nodes: _this.nodes,
              links: _this.links
          ...
          const param = {venue: this.venue}
          getAuthorsByVenue(param).then(res => {
          ...
          const param = {venue: this.venue}
          getPaperInVenueIsCitedTimes(param).then(res => {
