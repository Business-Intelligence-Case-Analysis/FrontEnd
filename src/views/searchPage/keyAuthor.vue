<template>
  <div>
    <div>
      <div style="font-weight: bolder;font-size: 32px">关键作者查询</div>
    ...
    initD3 () {
      ...
      getInfluentialAuthors(params).then(res => {
        ...
        for (let i = 0; i < res.data.value.length; i++) {
          ...
          totalList.push(res.data.value[i].pc)
          citeList.push(res.data.value[i].cn)
          highCiteList.push(res.data.value[i].hi)
          piList.push(res.data.value[i].pi)
          upiList.push(res.data.value[i].upi)
        }
        // 数据——对应y轴的值——城市人口（万）
        let dataList = []
        if (this.chooseValue === 'pc') dataList = totalList
        else if (this.chooseValue === 'cn') dataList = citeList
        else if (this.chooseValue === 'hi') dataList = highCiteList
        else if (this.chooseValue === 'pi') dataList = piList
        else if (this.chooseValue === 'upi') dataList = upiList
        // 画布的参数
        let mapWidth = 500
        let mapHeight = 500
        let mapPadding = 30
