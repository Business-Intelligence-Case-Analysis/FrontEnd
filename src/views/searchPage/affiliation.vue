  ...
  <div>
    <div>
      ...
      <div style="display: flex;margin-top: 20px">
        ...
        <el-input v-model="affiliation" clearable style="width: 150px;margin-left: 10px"></el-input>
...
import {getAuthorsInAffiliation} from '../../api/affiliationAPI'
...
export default {
  ...
  mounted () {
    this.setTypeColor()
    this.affiliation = sessionStorage.getItem('affiliation')
  ...
  methods: {
    ...
    search () {
      if ((this.affiliation === '') || (this.affiliation == null)) {
    ...
    initD3 () {
      ...
      _this.$nextTick(() => {
        ...
        if (this.chooseValue === '1') {
          const params = {affiliation: this.affiliation}
          getAuthorsInAffiliation(params).then(res => {
            ...
            _this.nodes.push({id: 1, name: this.affiliation, label: 'affiliation'})
            _this.links = res.data.value
            for (let i = 0; i < _this.links.length; i++) {
              _this.nodes.push(_this.links[i])
              _this.links[i]['target'] = affiliationId
